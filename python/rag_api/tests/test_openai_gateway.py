...
class OpenAIGatewayTests(unittest.TestCase):
    ...
    def test_guard_query_allows_in_scope_study_abroad_queries(self) -> None:
        ...
        self.assertIn("Use needs_context only when the latest input cannot be judged alone", prompt)
        self.assertIn("Do not use needs_context for complete questions", prompt)
        ...
        self.assertIn("A bare \"DSU\" is standalone enough to pass", prompt)
        ...
        self.assertIn('"виза"', prompt)
        self.assertIn("How to apply for visa", prompt)
        self.assertIn("How do I get visa?", prompt)
        self.assertIn("residence permit", prompt)
        self.assertIn("How do I get a residence permit", prompt)
        self.assertIn("How do I get residence permit", prompt)
        self.assertIn("How do I renew residence permit", prompt)
        ...
        self.assertIn("Как оформить ВНЖ", prompt)
        self.assertIn("тұруға рұқсат", prompt)
        self.assertIn("тұруға ықтиярхатты қалай аламын", prompt)
        ...
        self.assertIn("Book me a hotel in Rome", prompt)
        self.assertIn("Find cheap flights to Milan", prompt)
        self.assertIn("Can you plan my Rome vacation?", prompt)
        self.assertIn("How can I get a work visa in Italy?", prompt)
    ...
    def test_clarify_or_rewrite_query_returns_standalone_query_when_clear(self) -> None:
        ...
        self.assertIn("For a generic visa query without explicit non-study context, assume Italian student visa", prompt)
